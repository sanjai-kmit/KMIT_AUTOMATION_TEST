...
public class KMWARE_Orders extends Test_Base {
    ...
    public static String salesTransactionId;
        ...
        try {
            ...
            while (kmware_resultSet.next()){
                ...
                try {
                    ...
                    if (!act_billing_state.isEmpty()){
                        Log.info("[ec_OrderAddress] Tested Billing State: " + act_billing_state);
                    } else {
                        Log.info("[ec_OrderAddress] Billing State is empty");
                    }
                } catch (Throwable e){
                ...
                try {
                    ...
                    if (!act_billing_country.isEmpty()){
        ...
        try {
            ...
            while (kmware_resultSet.next()){
                ...
                try {
                    ...
                    if (!act_mailing_state.isEmpty()){
                        Log.info("[ec_OrderAddress] Tested Mailing State: " + act_mailing_state);
                    }else {
                        Log.error("[ec_OrderAddress] Mailing State Is Empty.");
                    }
                } catch (Throwable e){
                ...
                try {
                    ...
                    if (!act_mailing_country.isEmpty()){
    ...
    public void ec_OrderSalesTransaction(){
        String qry_select = "SELECT * FROM ec_OrderSalesTransaction WHERE OrderId = '" + order_number + "'";
        Log.info("[ec_OrderSalesTransaction] Query Selected for: " + qry_select);

        try {
            kmware_resultSet = kmware_statement.executeQuery(qry_select);

            while (kmware_resultSet.next()){
                salesTransactionId = kmware_resultSet.getString("SalesTransactionId");
                Log.info("[ec_OrderSalesTransaction] SalesTransactionId is: " + salesTransactionId);

                if (itemTitle.equals("MEMBERSHIP")){
                    Assert.assertEquals(kmware_resultSet.getString("ModuleCode").trim().replace("          ", ""), "MEMBERSHIP");
                } else {
                    Assert.assertEquals(kmware_resultSet.getString("ModuleCode").trim().replace("           ", ""), "ADMISSION");
                }
                Log.info("[ec_OrderSalesTransaction] ModuleCode is: " + itemTitle);

                membershipCode = kmware_resultSet.getString("MembershipCode");
                Log.info("[ec_OrderSalesTransaction] MembershipCode for: " + membershipCode);
            }
        }catch (SQLException e){
            Log.error("[ec_OrderSalesTransaction]: Exception on getting details of ec_OrderSalesTransaction table: " + e.getMessage());
        }
    }
}
